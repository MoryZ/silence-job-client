...
public abstract class AbstractScriptExecutor {
    ...
    private static final String WORKER_DIR = SilenceFileUtils.workspace() + "/script_processor/";
    ...
    protected ExecuteResult process(BigInteger jobId, ScriptParams scriptParams) {
        ...
        if (SilenceJobSystemUtils.isOsWindows() && SH_SHELL.equals(getRunCommand())) {
            logWarn("Current OS is {} where shell scripts cannot run.", SilenceJobSystemUtils.getOsName());
            return ExecuteResult.failure("Shell scripts cannot run on Windows.");
        }

        if (!SilenceJobSystemUtils.isOsWindows()) {
    ...
    private String prepareScriptFile(BigInteger jobId, ScriptParams scriptParams) {
        ...
        switch (scriptParams.getMethod()) {
            ...
            case SCRIPT_SCRIPT_CODE_METHOD:
                ...
                try {
                    ...
                    throw new SilenceJobInnerExecutorException("[silence-job] Failed to write script", e);
                }
                return scriptPath;
            default:
                throw new SilenceJobInnerExecutorException("[silence-job] Please correctly choose the script execution method.");
        }
    }
    ...
    private String handleLocalScript(File script, String scriptPath, String processorInfo) {
        ...
        if (routhFile.exists()) {
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(routhFile));
                ...
                throw new SilenceJobInnerExecutorException("[silence-job] Local script write exception", e);
            }
            return scriptPath;
        } else {
            throw new SilenceJobInnerExecutorException("File not found: {" + processorInfo + "}");
        }
    }
    ...
    private Charset getScriptChartset(ScriptParams scriptParams) {
        ...
        if (StrUtil.isNotBlank(charsetName)) {
            try {
                ...
                logWarn("[silence-job] Invalid charset:{} . Using default charset.", charsetName);
            }
        }
        return getCharset();
    }
    ...
    private void setScriptPermissions(String scriptPath) {
        ...
        try {
            ...
            throw new SilenceJobInnerExecutorException("[silence-job] Failed to set script permissions", e);
        }
    ...
    private ExecuteResult executeScript(String scriptPath, ScriptParams scriptParams) {
        ...
        try {
            ...
            throw new SilenceJobInnerExecutorException("[silence-job] Script execution failed", e);
        } finally {
            if (process.isAlive()) {
                // 脚本执行失败 终止;
                process.destroy();
    ...
    protected abstract String getScriptName(BigInteger instanceId);
    ...
    private void logInfo(String msg, Object... params) {
        SilenceJobLog.REMOTE.info("[silence-job] " + msg, params);
    }

    private void logWarn(String msg, Object... params) {
        SilenceJobLog.REMOTE.warn("[silence-job] " + msg, params);
    }

    public static class SilenceFileUtils {
        ...
        public static String workspace() {
            ...
            String workspaceByDKey = silenceJobProperties.getWorkspace();
