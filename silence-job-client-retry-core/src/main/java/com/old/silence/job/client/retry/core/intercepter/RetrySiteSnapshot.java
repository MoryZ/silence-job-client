...
public class RetrySiteSnapshot {
     ...
     */
    private static final RetrySiteSnapshotContext<Integer> RETRY_STAGE = SilenceRetrySpiLoader.loadRetrySiteSnapshotContext();

    /**
     * 标记重试方法入口
     */
    private static final RetrySiteSnapshotContext<Deque<MethodEntranceMeta>> RETRY_CLASS_METHOD_ENTRANCE = SilenceRetrySpiLoader.loadRetrySiteSnapshotContext();

    /**
     * 重试状态
     */
    private static final RetrySiteSnapshotContext<Integer> RETRY_STATUS = SilenceRetrySpiLoader.loadRetrySiteSnapshotContext();

    /**
     * 重试请求头
     */
    private static final RetrySiteSnapshotContext<SilenceJobHeaders> RETRY_HEADER = SilenceRetrySpiLoader.loadRetrySiteSnapshotContext();

    /**
     * 状态码
     */
    private static final RetrySiteSnapshotContext<String> RETRY_STATUS_CODE = SilenceRetrySpiLoader.loadRetrySiteSnapshotContext();

    /**
     * 进入方法入口时间标记
     */
    private static final RetrySiteSnapshotContext<Long> ENTRY_METHOD_TIME = SilenceRetrySpiLoader.loadRetrySiteSnapshotContext();
    private static final RetrySiteSnapshotContext<Integer> ATTEMPT_NUMBER = SilenceRetrySpiLoader.loadRetrySiteSnapshotContext();
    ...
    public static SilenceJobHeaders getRetryHeader() {
        return RETRY_HEADER.get();
    }

    public static void setRetryHeader(SilenceJobHeaders headers) {
    ...
    public static boolean isRetryFlow() {
        SilenceJobHeaders retryHeader = getRetryHeader();
    ...
    public static boolean isRetryForStatusCode() {
        return Objects.nonNull(getRetryStatusCode()) && getRetryStatusCode()
                .equals(SystemConstants.SILENCE_JOB_STATUS_CODE);
    }
    ...
    public enum EnumStage {
        ...
        public static EnumStage valueOfStage(int stage) {
            ...
            throw new SilenceRetryClientException("unsupported stage");
        }
